...
| `bashrc` | 사용자 홈에 `.bashrc`로 mount되는 기본 셸 설정 템플릿이다. 로그아웃 감지와 세션 모니터링 함수를 포함한다. | Pod account-files volume subPath `bashrc` | 게스트 셸 alias, trap, monitor log |
...
이 디렉토리에는 Python 클래스나 함수 정의가 없다. Shell 함수는 `bashrc`의 `run_logout_once`, `safe_logout`, `kst_date`, `ssh_session_monitor`, `check_session`과 `bash.bash_logout`의 `on_farewell_custom`이 있으며, 모두 사용자 SSH 세션 종료 감지와 로그 기록을 위한 함수이다.
